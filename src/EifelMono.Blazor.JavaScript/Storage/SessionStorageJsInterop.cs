...
{
    public static class SessionStorageJsInterop // www/Storage/SessionStorageJSInterop.js
    {
        internal static string s_jSInteropName = "EifelMonoBlazorJavaScriptSessionStorageJSInterop";
        ...
        public static async ValueTask<(bool Ok, bool Value)> KeyExistsSafeAsync(string key)
        {
            try
            {
                return (true, await Core.HostHtmlInit.JSRuntime.InvokeAsync<bool>($"{s_jSInteropName}.exists", key).ConfigureAwait(false));
            }
            catch (Exception ex)
            {
                ex.LogException();
        ...
        public static async ValueTask<(bool Ok, bool Value)> SetValueSafeAsync<T>(string key, T value)
        {
            try
            {
                return (true, await Init.JSRuntime.InvokeAsync<bool>($"{s_jSInteropName}.set", key, value).ConfigureAwait(false));
            }
            catch (Exception ex)
            {
                ex.LogException();
        ...
        public static async ValueTask<(bool Ok, bool Value)> DeleteKeySafeAsync(string key)
        {
            try
            {
                return (true, await Init.JSRuntime.InvokeAsync<bool>($"{s_jSInteropName}.delete", key).ConfigureAwait(false));
            }
            catch (Exception ex)
            {
                ex.LogException();
